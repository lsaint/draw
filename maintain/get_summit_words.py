...
class SummitWordsDao(object):
    ...
    def LoadSummitWords(self):
        ...
        sql = "select id,word_str,word_number,word_category,commit_uid,commit_name,channel_id,create_time from app_pending_summit_draw_dictionary"
        if self._mysql.Execute(sql, None):
            cursor = self._mysql.Cursor()
            rs = cursor.fetchall()
            if len(rs) != 0:
                for row in rs:
                    if len(row) == 8:#随着上面sql语句里面的field数量而变化
